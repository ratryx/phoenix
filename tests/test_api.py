...
def test_api_protection_state_propagation(monkeypatch):
    from modules.gui.api import PhoenixAPI
    from modules.gui.jobs import JobManager

    class MockRoutineService2:
        def __init__(self):
            self.last_state = None
        def executar(self, id_atendimento, nome_cliente, job_context=None, protection_state=None):
            self.last_state = protection_state
            return {"ok": True, "res": "mockado"}

    mock_routine = MockRoutineService2()
    manager = JobManager()

    api = PhoenixAPI({}, job_manager=manager, routine_service=mock_routine)

    # Test restore_created
    api._protection_state = "restore_created"
    res = api.executar_rotina_completa(nome_cliente="Fulano")

    import time
    time.sleep(0.1)

    assert mock_routine.last_state == {"status": "restore_created"}

    # Test risk_accepted
    api._protection_state = "risk_accepted"
    res = api.executar_rotina_completa(nome_cliente="Fulano2")

    time.sleep(0.1)
    assert mock_routine.last_state == {"status": "risk_accepted"}
