...
def test_hardware_service_metricas_completas():
    ...
    # Precisamos mockar datetime pra nÃ£o dar erro no boot time real vs timestamp 1600000000?
...
class SlowMockHardwareMod:
    ...
    def coletar_hardware_completo(self, progress_callback=None):
        ...
        return {"fake": "scan_slow"}

def test_hardware_service_single_flight():
    mock_hw = SlowMockHardwareMod()
    ...
    # Ambos devem receber o mesmo resultado
    assert len(results) == 2
    assert results[0] == results[1]
    assert results[0]["ok"] is True
    assert results[0]["hardware"]["fake"] == "scan_slow"

    # Coleta de fato só foi chamada uma vez
    assert mock_hw.call_count == 1

def test_hardware_service_single_flight_failure():
    mock_hw = SlowMockHardwareMod(should_fail=True)
    svc = HardwareService(hardware_mod=mock_hw)
    ...
    # Ambos devem receber o mesmo erro
    assert len(results) == 2
    assert results[0] == results[1]
    ...
    # O lock foi liberado e o estado limpo para a próxima tentativa
